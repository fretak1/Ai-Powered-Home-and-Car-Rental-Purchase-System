...
import { FiltersState, setFilters, toggleFiltersFullOpen, setViewMode } from "@/state";
...
const FiltersBar = () => {
  ...
  const pathname = usePathname()SelectItem: JSX.IntrinsicAttributes, ;
  const filters = useAppSelector((state: { global: { filters: any; }; }) => state.global.filters);
  const isFiltersFullOpen = useAppSelector(
    (state) => state.global.isFiltersFullOpen
  );
  const viewMode = useAppSelector(
    (state: { global: { viewMode: any } }) => state.global.viewMode
  );
  ...
  function handleLocationSearch(event: MouseEvent<HTMLButtonElement, MouseEvent>): void {
    throw new Error("Function not implemented.");
  }
